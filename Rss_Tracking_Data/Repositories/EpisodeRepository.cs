...
namespace Rss_Tracking_Data.Repositories
{
    ...
    public class EpisodeRepository : BaseRepository<Episode>, IEpisodeRepository
    {
        ...
        public override Episode? GetById(Guid id) => _context.Episodes.Where(x => x.Id == id).FirstOrDefault();
        public override List<Episode> GetAll() => _context.Episodes.ToList();
        ...
        public override bool AlreadyExists(Episode episode) => _context.Episodes.Any(x => x.EpisodeId == episode.EpisodeId && x.Url == episode.Url && x.Description == episode.Description && x.ThumbnailUrl == episode.ThumbnailUrl && x.Name == episode.Name);
